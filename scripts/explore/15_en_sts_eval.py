"""
scripts/explore/15_en_sts_eval.py
STSBenchmark(영어)로 임베딩 모델의 의미 유사도 성능을 평가합니다.

평가 방식:
  - STSBenchmark test split (1,379쌍) 사용
  - 각 문장쌍의 임베딩 → 코사인 유사도 계산
  - 사람이 붙인 유사도 정답(0~5점)과 Spearman 상관계수 비교

실행 예:
  uv run python scripts/explore/15_en_sts_eval.py model=bge_large_en
  uv run python scripts/explore/15_en_sts_eval.py model=mxbai_large
  uv run python scripts/explore/15_en_sts_eval.py model=bge_base_en
  uv run python scripts/explore/15_en_sts_eval.py model=mpnet_base
  uv run python scripts/explore/15_en_sts_eval.py model=bge_m3
"""
...
def main(cfg: DictConfig) -> None:
    ...
    sep = "=" * 55
    print(f"\n{sep}")
    print(f"  STSBenchmark 평가 결과  |  모델: {model_name}")
    print(sep)
    print(f"  Spearman ρ   : {spearman_r:.4f}  (p={spearman_p:.2e})")
    print(f"  Pearson  r   : {pearson_r:.4f}  (p={pearson_p:.2e})")
    print(f"\n  ▶ 해석:")
    print(f"    0.87↑: SOTA 수준")
    print(f"    0.84↑: 우수")
    print(f"    0.80↑: 양호")
    print(f"    0.75↓: 개선 필요")
    print(sep)
    ...
    for idx in np.argsort(errors)[:5]:
        ...
        print(f"    A: {sents_a[idx][:60]}")
        print(f"    B: {sents_b[idx][:60]}")
        print()
